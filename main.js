...
request.get({
    ...
    try {
            ...
            if(body.message != "Unknown Gift Code" && body.message != "You are being rate limited." && body.message != "404: Not Found"){
                console.log(colors.green(`[CHECKER] Code fonctionnel : https://discord.gift/${code}`));
                fs.appendFileSync("working.txt", code+'\n', "UTF-8",{'flags': 'a+'});
                i++
                a++
            }
            else {
                console.log(colors.red(`${code} est invalide !` + colors.white(`${debug ? `\n[DEBUG] Proxy: ${proxy}\n[DEBUG] Message: ${body.message}` : ''}`)));
                i++
            }
        }
        catch (error) {
...
request.get({
    ...
    try {
      ...
      if (!noerrormessages) {
        return console.log(colors.red(`Le proxy ne fonctionne pas : ${err}`));
      }
        }
});
}
